...
const createBinTimer = (binTimers: Storage.BinTimers, ttl: number, binId: RequestCatcher.BinId, callback: () => void) => {
  binTimers[binId] = setTimeout(callback, ttl * 1000)
}

export const isBinExpired = (bin: RequestCatcher.Bin): boolean => {
  return null !== bin.expire_at && moment().utc().isAfter(bin.expire_at)
}

export const createBinConfigData = (binTimers: Storage.BinTimers, createdAt: string, requestsCount: number, binConfig: RequestCatcher.BinConfig, binId: RequestCatcher.BinId, removeCallback: () => void): RequestCatcher.Bin => {
  ...
  if (ttl > 0) {
    ...
    expiresAt = moment.utc().add(ttl, 'seconds').toISOString()
