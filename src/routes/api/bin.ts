...
const applyDefault = (defaultBody: RequestCatcher.InputBinConfig): RequestHandler => (request, _response, next) => {
  request.body = merge.recursive(true, defaultBody, request.body)
  return next()
}

export const addBinRoutes = (apiRouter: Router) => {

  // Create bin
  apiRouter.post('/request-bin', validate(validation.api.createBin), applyDefault(defaultBinConfig), async (request, response, next) => {

    console.log(request.t('welcome', {ns: 'general'}))
